...
function ninjastate.new()
	...
	function state:init()
		...
		header = TextBox.new("LiberationSansNarrow-Regular", 24)
		header:setColor(1, 1, 1, 1)
		header:setRect(virtualWidth/16, virtualHeight/16, virtualWidth - virtualWidth/16, virtualHeight/4)
		header:setAlignment(MOAITextBox.CENTER_JUSTIFY, MOAITextBox.CENTER_JUSTIFY)
		header:setString('telnet to debug server on port 12345\nyou can send  jump() attack() quit()')
		...
		-- this will expose the action map to others to make it easier to inject actions
	...
	function state:gotFocus()
		...
		header:insertIntoLayer(mainlayer)
