...
local imguiteststate = {}

function imguiteststate.new()

	local state = {}

	state.name = "ImGuiTestState"
				...
				MOAIImGui.Begin("Window Test")
					...
					MOAIImGui.BeginChildFrame(128, v)
						...
						if MOAIImGui.TreeNode("id2", "Buttons") then
							...
							_, checked = MOAIImGui.Checkbox("Checkbox", checked)
							if checked then
								MOAIImGui.Text("Checked! ")
							end
							_, activebutton = MOAIImGui.RadioButton("Radio 1", activebutton, 1)
							_, activebutton = MOAIImGui.RadioButton("Radio 2", activebutton, 2)
							_, activebutton = MOAIImGui.RadioButton("Radio 3", activebutton, 3)
							_, activebutton = MOAIImGui.RadioButton("Radio 4", activebutton, 4)
						...
						if MOAIImGui.TreeNode("Drag Numbers") then
							_, comboitem = MOAIImGui.Combo("Numbers", comboitem, comboitems)
							...
							_, progress = MOAIImGui.DragFloat("progress", progress, 0.01, 0, 1, nil, nil)
							...
							_, min, max = MOAIImGui.DragFloatRange2("Range", min, max)

							_, idrag = MOAIImGui.DragInt("idrag", idrag, 1, 0, 100, nil, nil)
							...
							_, imin, imax = MOAIImGui.DragIntRange2("iRange", imin, imax)
					...
					if MOAIImGui.TreeNode("Input Text") then
						local enter
						enter, name = MOAIImGui.InputText("What is your name?", name, 12)
					...
					if MOAIImGui.TreeNode("Slider Numbers") then
						...
						_, slide = MOAIImGui.SliderFloat("slider", slide, 0, 1, nil, nil)
						...
						MOAIImGui.Spacing()
						_, slideangle = MOAIImGui.SliderAngle("slider angle", slideangle, 0, 90)
						MOAIImGui.Spacing()

						_, islide = MOAIImGui.SliderInt("islide", islide, 0, 100, nil, nil)
						...
						_, slide = MOAIImGui.VSliderFloat("vslider", vslidesize, slide, 0, 1, nil, nil)
						MOAIImGui.SameLine()
						MOAIImGui.NewLine()
						MOAIImGui.SameLine()
						_, islide = MOAIImGui.VSliderInt("vislide", vslidesize, islide, 0, 100, nil, nil)

						MOAIImGui.TreePop()
					end

					local isopen
					isopen, showheader = MOAIImGui.CollapsingHeader("Input Numbers", showheader)
					if isopen then
						MOAIImGui.Columns(2, nil, false)
						_, f = MOAIImGui.InputFloat("InputFloat", f)
						...
						MOAIImGui.Spacing()
						_, num = MOAIImGui.InputInt("InputInt", num)
					...
					if MOAIImGui.CollapsingHeader("Selectables") then
						_, selected = MOAIImGui.Selectable("Select me", selected)

						_, listitem = MOAIImGui.ListBox("List", listitem, listitems)
					...
					if MOAIImGui.CollapsingHeader("Popups") then
						...
						if MOAIImGui.BeginPopup("select") then
							...
							for k, v in ipairs(fish) do
								local _, selected MOAIImGui.Selectable(fish[k])
								if selected then
									fishitem = k
								end
							end
