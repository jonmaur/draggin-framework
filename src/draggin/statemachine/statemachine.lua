--[[
Originally started from AS3-FSM Copyright (c) 2009-10 Cássio S. Antonio
Later ported to the Lua Corona SDK by Peter Cardwell-Gardner
And finally ported to the MOAI SDK with the Draggin Framework by Jon Maur
]]

local State = require "draggin/statemachine/state"
...
function StateMachine.new(entity)
	...
	-- signals
	stateMachine.sig_onTransitionComplete = Signal.new()
	stateMachine.sig_onStateMachineStateChanged = Signal.new()
	stateMachine.sig_onTransitionDenied = Signal.new()
	stateMachine.sig_onTransitionDenied = Signal.new()
	...
	function stateMachine:setInitialState(stateName)
		...
		if self.state == nil and initial ~= nil then
			...
			local outEvent = {target = self, toState = stateName}
			self.sig_onTransitionComplete:emit(outEvent)
		end
	end
	...
	function stateMachine:setState(value)
		...
		self.sig_onStateMachineStateChanged:emit({target=self})
	end
	...
	function stateMachine:changeState(stateTo)
		...
		local toState = states[stateTo]
		if toState == nil then
			print("ERROR: StateMachine::changeState, Cannot make transition: from " .. state .. " to " .. stateTo .. " it's not defined.")
			return false
		end
		--print("before canChange")
		--print("btw, can we change?: ", self:canChangeStateTo(stateTo))
		if self:canChangeStateTo(stateTo) == false then
			print("ERROR: StateMachine::changestate, Transition from " .. state .. " to " .. stateTo .. " denied.")
